...
print(vec1[i]*vec2[j]for i in range(len(vec1)) for j in range(len(vec2)))
...
# 列表推导式可以使用复杂表达式或嵌套函数：
Rd = [str(round(355/113, i)) for i in range(1, 6)]
print(Rd)
...
abd = dict([('sape', 4139), ('guido', 4127), ('jack', 4098)])
print(abd)
...
aen = dict(sape=4139, guido=4127, jack=4098)
print(aen)
...
for k ,l in knights.items():
    print(k, l)

# 在序列中遍历时，索引位置和对应值可以使用 enumerate() 函数同时得到：
listr = ['tic', 'tac', 'toe']
for i in enumerate(listr):
    print(i)
